...
// PERBAIKAN IMPORT (Titik hanya satu tingkat karena posisi Sidebar)
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user && user.email) {
        try {
          const q = query(collection(db, "users_desa"), where("email", "==", user.email));
  ...
  const menuList = [
    ...
    { 
      ...
      sub: [
        ...
        { id: "profil-lembaga", label: "Lembaga Desa" },
  ...
  return (
    <aside className="w-64 bg-gray-900 text-white min-h-screen fixed top-0 left-0 overflow-y-auto hidden lg:block shadow-2xl z-50">
      
      <div className="p-6 flex items-center gap-3 border-b border-gray-800 bg-gray-950 sticky top-0 z-10">
        <div className="w-10 h-10 bg-white rounded-full p-1 flex-shrink-0 shadow-lg">
          <img 
        ...
        <div>
          <h2 className="font-black text-lg tracking-wide text-white leading-tight">
            Desa Kerjo
          </h2>
      ...
      <nav className="p-4 space-y-2 mt-2 pb-24">
          ...
          return (
            ...
            >
              ...
              >
                ...
                {hasSub && (
                  <span className={`text-[10px] transform transition-transform duration-300 ease-in-out ${
                    isHovered || isActive 
              ...
              {hasSub && (
                ...
                >
                  <div className="pl-12 flex flex-col gap-1 border-l-2 border-gray-800 ml-6 relative">
                    {item.sub.map((subItem, idx) => (
                      ...
                      >
                        <span className="absolute left-[-2px] top-1/2 w-3 h-[2px] bg-gray-700"></span>
                        {subItem.label}
                      </Link>
                    ))}
                  </div>
      ...
      <div className="absolute bottom-0 w-full bg-gray-950 p-5 border-t border-gray-800">
