    ...
    <nav className="bg-white shadow-md sticky top-0 z-50">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          {/* LOGO & NAMA DESA (SUDAH DIUBAH MENJADI GAMBAR) */}
          <div className="flex-shrink-0 flex items-center gap-3">
            <div className="w-12 h-12 flex items-center justify-center overflow-hidden">
              {/* GANTI TULISAN DI BAWAH INI DENGAN LINK GAMBAR LOGO DESA ANDA */}
              <img 
                src="https://ibb.co.com/LXwxvhWk" 
                alt="Logo Desa" 
                className="w-full h-full object-contain"
                // Opsional: Jika link belum diisi, kita tampilkan logo placeholder sementara
                  ...
                  <div className="absolute left-0 mt-4 w-56 bg-white border border-gray-100 shadow-xl rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 overflow-hidden">
                    <div className="p-2 flex flex-col">
                      {item.sub.map((subItem, idx) => (
                        <Link key={idx} href={subItem.link} className="px-4 py-3 text-sm font-semibold text-gray-600 hover:text-green-700 hover:bg-green-50 rounded-xl transition-colors">
                          {subItem.title}
                        </Link>
                      ))}
                    </div>
            ...
            {navItems.map((item, index) => (
              <div key={index} className="flex flex-col">
                <div className="flex justify-between items-center px-4 py-3 bg-gray-50 rounded-xl">
                  <Link href={item.path} onClick={() => setIsMobileMenuOpen(false)} className="font-bold text-gray-800 flex-grow">
                    {item.name}
                  </Link>
                  {item.sub.length > 0 && (
                ...
                {item.sub.length > 0 && openDropdownMobile === item.name && (
                  <div className="pl-6 pr-4 py-2 space-y-1 mt-1 border-l-2 border-green-500 ml-4">
                    {item.sub.map((subItem, idx) => (
                      <Link key={idx} href={subItem.link} onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-3 text-sm font-semibold text-gray-600 hover:text-green-700 hover:bg-green-100 rounded-lg">
                        - {subItem.title}
                      </Link>
                    ))}
                  </div>
