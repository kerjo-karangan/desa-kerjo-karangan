...
export default function ProfilUmkm() {
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      ...
      {/* 4. MODUL KATALOG UMKM & WISATA DESA (PERBAIKAN RESPONSIVITAS JAM DI HP) */}
              ...
              <div className="bg-white p-3 rounded-xl border border-yellow-300 shadow-sm">
                ...
                <div className="space-y-3">
                  {Object.keys(jamOperasional).map((hari) => (
                    <div key={hari} className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between bg-gray-50 p-2.5 rounded-lg border border-gray-100">
                      
                      {/* Nama Hari & Ceklis Libur */}
                      ...
                      {/* Input Waktu (Turun ke bawah di layar kecil) */}
                      {!jamOperasional[hari].libur ? (
                        <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto mt-1 sm:mt-0">
                          <input type="time" value={jamOperasional[hari].buka} onChange={(e) => handleJamChange(hari, 'buka', e.target.value)} className="w-full sm:w-auto border border-gray-300 p-1.5 text-xs rounded outline-none focus:border-yellow-500 bg-white" />
                          <span className="text-gray-400 font-bold">-</span>
                          <input type="time" value={jamOperasional[hari].tutup} onChange={(e) => handleJamChange(hari, 'tutup', e.target.value)} className="w-full sm:w-auto border border-gray-300 p-1.5 text-xs rounded outline-none focus:border-yellow-500 bg-white" />
                        </div>
                      ) : (
                         <div className="w-full sm:w-32 text-center text-xs font-bold text-red-500 bg-red-100 rounded py-1.5 mt-1 sm:mt-0 border border-red-200">TUTUP / LIBUR</div>
                      )}
                    </div>
                  ))}
                </div>
