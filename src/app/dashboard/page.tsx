...
export default function DashboardAdmin() {
  ...
  const handleSimpanAkun = async (e: React.FormEvent) => {
    ...
    setStatusAkun("Mendaftarkan Biodata Akun...");
    try {
      // Menyimpan data role ke database khusus "users_role"
      ...
      setStatusAkun("✅ Data Hak Akses tersimpan! Penting: Anda tetap harus membuatkan password untuk email ini di menu Authentication Firebase Console.");
      setNamaAkun(""); setEmailAkun(""); setRoleAkun("Kontributor");
    } catch (error) {
      setStatusAkun("❌ Gagal menyimpan data akun.");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col md:flex-row font-sans">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        {/* MODUL MANAJEMEN AKUN (BARU) */}
        {activeMenu === "akun" && (
          <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border-t-4 border-blue-500 animate-fade-in">
            <h3 className="text-2xl font-bold text-gray-800 mb-2 flex items-center gap-2">
              <span className="text-3xl">👥</span> Hak Akses Sistem
            </h3>
            <p className="text-gray-500 mb-6 text-sm">Tambahkan identitas pengguna baru. Ingat, *password* tetap harus dibuat secara manual di Firebase Authentication demi keamanan.</p>
            
            <form onSubmit={handleSimpanAkun} className="space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Nama Pengguna</label>
                  <input type="text" required value={namaAkun} onChange={(e) => setNamaAkun(e.target.value)} placeholder="Contoh: Budi Santoso" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Email Terdaftar</label>
                  <input type="email" required value={emailAkun} onChange={(e) => setEmailAkun(e.target.value)} placeholder="budi@kerjo.desa.id" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none" />
                </div>
              </div>
              
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Jabatan / Role</label>
                <select value={roleAkun} onChange={(e) => setRoleAkun(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none bg-white cursor-pointer font-medium text-gray-700">
                  <option value="Kontributor">Kontributor (Hanya bisa tambah berita)</option>
                  <option value="Administrator">Administrator (Kendali penuh)</option>
                </select>
              </div>

              {statusAkun && (
                <div className={`p-4 rounded-xl text-sm font-bold border ${statusAkun.includes("❌") ? "bg-red-50 text-red-600 border-red-200" : "bg-blue-50 text-blue-700 border-blue-200"}`}>
                  {statusAkun}
                </div>
              )}

              <button type="submit" disabled={isLoadingAkun} className={`w-full md:w-auto px-8 py-3 rounded-xl font-bold text-white shadow-md transition-all ${isLoadingAkun ? "bg-gray-400" : "bg-blue-600 hover:bg-blue-700 hover:-translate-y-1"}`}>
                {isLoadingAkun ? "Menyimpan..." : "Daftarkan Pengguna"}
              </button>
            </form>
          </div>
        )}
