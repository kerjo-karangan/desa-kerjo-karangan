...
export default function DashboardAdmin() {
  ...
  const uploadFotoKeImgBB = async (file: File) => {
    ...
    try {
      ...
      console.warn("Mencoba Jalur CDN CORS Proxy...");
      try {
  ...
  // ==========================================
  // MODUL PROFIL & APARATUR DESA (DENGAN EDIT)
  // ==========================================
  ...
  const handleSimpanAparatur = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fotoAparatur) { 
        setStatusAparatur("Mengunggah foto..."); 
  ...
  // ==========================================
  // MODUL TRANSPARANSI & REGULASI (BARU)
  // ==========================================
  ...
  const handleSimpanRegulasi = async (e: React.FormEvent) => {
    ...
    try {
      if (editRegulasiId) {
        ...
        setStatusRegulasi("✅ Dokumen diperbarui!");
      } else {
        ...
        setStatusRegulasi("✅ Dokumen berhasil ditambahkan!");
      }
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        {activeMenu === "welcome" && ( 
          <div className="animate-fade-in space-y-6">
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 bg-gradient-to-r from-white to-green-50"><h3 className="text-3xl font-extrabold text-green-900 mb-2">Selamat Datang, Admin!</h3><p className="text-gray-600 font-medium">Sistem Informasi Desa Kerjo berjalan normal. Berikut adalah ringkasan data saat ini:</p></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white p-6 rounded-3xl shadow-sm border-t-4 border-yellow-500 flex flex-col justify-center items-center transform hover:scale-105 transition-transform"><span className="text-4xl mb-3">✉️</span><h4 className="text-gray-500 font-bold uppercase tracking-wider text-xs">Total Permohonan Surat</h4><p className="text-4xl font-black text-gray-900 mt-2">{daftarSurat.length}</p>{daftarSurat.filter(s => s.status_berkas === "Diajukan").length > 0 && <p className="text-red-500 text-xs font-bold mt-2 animate-pulse">{daftarSurat.filter(s => s.status_berkas === "Diajukan").length} Menunggu Diproses!</p>}</div>
              <div className="bg-white p-6 rounded-3xl shadow-sm border-t-4 border-green-500 flex flex-col justify-center items-center transform hover:scale-105 transition-transform"><span className="text-4xl mb-3">📰</span><h4 className="text-gray-500 font-bold uppercase tracking-wider text-xs">Total Berita Dipublikasi</h4><p className="text-4xl font-black text-gray-900 mt-2">{riwayatKabar.length}</p></div>
              <div className="bg-white p-6 rounded-3xl shadow-sm border-t-4 border-blue-500 flex flex-col justify-center items-center transform hover:scale-105 transition-transform"><span className="text-4xl mb-3">📢</span><h4 className="text-gray-500 font-bold uppercase tracking-wider text-xs">Total Pengaduan Masuk</h4><p className="text-4xl font-black text-gray-900 mt-2">{daftarPengaduan.length}</p></div>
            </div>
          </div> 
        )}

        {/* MODUL LAYANAN WARGA (MEMPERBAIKI ERROR KLIK FOTO PENGADUAN) */}
        {activeMenu === "layanan" && ( 
                ...
                {daftarPengaduan.map((lap) => (
                  <div key={lap.id} className="border p-5 rounded-2xl flex justify-between bg-gray-50 relative">
                    ...
                    <div className="flex gap-4">
                      {lap.foto_bukti && (
                        // PERBAIKAN LINK FOTO: Memasukkan Link ke dalam Proxy wsrv.nl agar saat diklik Tab Baru tidak Error
                        <a href={`https://wsrv.nl/?url=${lap.foto_bukti}`} target="_blank" rel="noreferrer" className="w-20 h-20 rounded-xl overflow-hidden border border-gray-300 block hover:opacity-80">
                          <img src={`https://wsrv.nl/?url=${lap.foto_bukti}`} alt="Bukti" className="w-full h-full object-cover" />
                        </a>
                      )}
                      <div>
        ...
        {/* MODUL PROFIL (SEJARAH & APARATUR DESA YG SUDAH DIPERBAIKI) */}
        {activeMenu === "profil" && ( 
          <div className="space-y-8 animate-fade-in">
            ...
            {/* MANAJEMEN APARATUR (TAMBAH FITUR EDIT + UPLOAD BUTTON) */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 bg-gray-50 p-6 rounded-2xl border border-gray-200">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="font-bold text-gray-800">{editAparaturId ? "✏️ Edit SOTK" : "Tambah SOTK"}</h4>
                    {editAparaturId && <button onClick={batalEditAparatur} className="text-xs bg-gray-300 px-2 py-1 rounded-md font-bold">Batal</button>}
                  </div>
                  ...
                  <form onSubmit={handleSimpanAparatur} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {/* DESAIN UPLOAD FOTO ELEGAN (BUKAN TEXT LAGI) */}
                      <div>
                        <label className="block text-xs font-bold mb-1">Foto Baru</label>
                        <label className="cursor-pointer flex flex-col items-center justify-center py-2 h-[46px] bg-white border border-gray-300 rounded-lg hover:bg-gray-100 transition-all overflow-hidden">
                          <span className="font-bold text-gray-600 text-xs flex items-center gap-1">📸 {fotoAparatur ? 'Siap' : 'Pilih File'}</span>
                    ...
                    {editAparaturId && fotoLamaAparatur && (
                      <div className="flex items-center gap-3 p-2 bg-white rounded-lg border">
                        <img src={`https://wsrv.nl/?url=${fotoLamaAparatur}`} className="w-8 h-8 rounded-full object-cover" />
                    ...
                    <tbody>{daftarAparatur.map((org) => (
                      <tr key={org.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-center">
                          <button onClick={() => mulaiEditAparatur(org)} className="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-md mr-2">Edit</button>
                          <button onClick={() => hapusAparatur(org.id)} className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-md">Hapus</button>
                        </td>
                      </tr>))}
                    </tbody>
                  </table>
        ...
        {/* MODUL TRANSPARANSI & REGULASI (TAHAP 22 - DENGAN FITUR TAMBAH PERDES) */}
        {activeMenu === "transparansi" && ( 
          <div className="space-y-8 animate-fade-in">
            ...
            {/* Bagian Manajemen Regulasi / Perdes */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
              <h3 className="text-2xl font-bold mb-6">📂 Manajemen Dokumen & Regulasi</h3>
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                
                {/* Form Tambah/Edit Regulasi */}
                <div className="lg:col-span-1 bg-gray-50 p-6 rounded-2xl border border-gray-200">
                  <div className="flex justify-between items-center mb-4">
                    <h4 className="font-bold text-gray-800">{editRegulasiId ? "✏️ Edit Dokumen" : "Tambah Dokumen"}</h4>
                    {editRegulasiId && <button onClick={batalEditRegulasi} className="text-xs bg-gray-300 px-2 py-1 rounded-md font-bold">Batal</button>}
                  </div>
                  <form onSubmit={handleSimpanRegulasi} className="space-y-4">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <div><label className="block text-xs font-bold mb-1">Jenis</label>
                        <select value={jenisRegulasi} onChange={(e)=>setJenisRegulasi(e.target.value)} className="w-full p-3 rounded-lg border outline-none bg-white">
                          <option value="Perdes">Perdes</option><option value="SK Kades">SK Kades</option><option value="Laporan">Laporan</option>
                        </select>
                      </div>
                    </div>
                    <div><label className="block text-xs font-bold mb-1">Judul Dokumen</label><textarea required rows={3} value={judulRegulasi} onChange={(e)=>setJudulRegulasi(e.target.value)} placeholder="Misal: Peraturan Desa tentang APBDes" className="w-full p-3 rounded-lg border outline-none"></textarea></div>
                    <div><label className="block text-xs font-bold mb-1">Tautan / Link File (G-Drive dll)</label><input type="url" required value={linkRegulasi} onChange={(e)=>setLinkRegulasi(e.target.value)} placeholder="https://drive.google.com/..." className="w-full p-3 rounded-lg border outline-none" /></div>
                ...
                {/* Tabel Daftar Regulasi */}
                <div className="lg:col-span-2 overflow-x-auto">
                  <table className="min-w-full text-sm text-left"><thead className="bg-gray-100 border-b"><tr><th className="py-3 px-4">Thn/Jenis</th><th className="py-3 px-4">Judul Dokumen</th><th className="py-3 px-4 text-center">Aksi</th></tr></thead>
                    <tbody>
                      {daftarRegulasi.map((docItem) => (
                        <tr key={docItem.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="py-3 px-4"><div className="font-bold text-gray-900 line-clamp-2">{docItem.judul}</div><a href={docItem.link} target="_blank" className="text-xs text-blue-500 hover:underline">Tes Link Berkas</a></td>
                          <td className="py-3 px-4 text-center flex flex-col gap-1">
                            <button onClick={() => mulaiEditRegulasi(docItem)} className="bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1 rounded-md">Edit</button>
                            <button onClick={() => hapusRegulasi(docItem.id)} className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-md">Hapus</button>
                          </td>
                        </tr>
                      ))}
