      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
        ...
        {activeMenu === "layanan" && (
          <div className="space-y-8 animate-fade-in">
            ...
            {/* TABEL KOTAK PENGADUAN DENGAN PROXY WSRV.NL */}
            <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {daftarPengaduan.map((laporan) => (
                  <div key={laporan.id} className="border border-gray-200 rounded-2xl p-5 bg-gray-50 relative">
                    <button onClick={() => hapusPengaduan(laporan.id)} className="absolute top-4 right-4 text-gray-400 hover:text-red-500 transition-colors" title="Hapus Laporan">✖</button>
                    <div className="flex gap-4">
                      {laporan.foto_bukti && (
                        <a href={`https://wsrv.nl/?url=${laporan.foto_bukti}`} target="_blank" rel="noreferrer" className="flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border border-gray-300 shadow-sm block hover:opacity-80">
                          {/* MENGGUNAKAN PROXY ANTI-BLOKIR DI DASHBOARD */}
                          <img src={`https://wsrv.nl/?url=${laporan.foto_bukti}`} alt="Bukti" className="w-full h-full object-cover" />
                        </a>
                      )}
                      <div>
                        <h4 className="font-bold text-gray-900 text-lg leading-tight mb-1">{laporan.judul}</h4>
                        <span className="text-xs text-gray-500 font-medium">{new Date(laporan.tanggal_laporan).toLocaleDateString('id-ID')} • {laporan.anonim ? "👤 Pelapor Anonim" : "Terverifikasi"}</span>
                        <p className="text-sm text-gray-700 mt-2 line-clamp-3">{laporan.isi}</p>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {/* MODUL LAMA TETAP UTUH (KABAR, PROFIL, TRANSPARANSI, AKUN) */}
        {activeMenu === "kabar" && (<div className="space-y-8 animate-fade-in"><div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border-t-4 border-green-500"><div className="flex justify-between mb-6 border-b pb-4"><h3 className="text-2xl font-bold flex items-center gap-2">{editKabarId ? "✏️ Edit Berita" : "📰 Publikasi Berita"}</h3>{editKabarId && <button onClick={batalEditKabar} className="bg-gray-200 px-4 py-2 rounded-lg font-bold">Batal Edit</button>}</div><form onSubmit={handleSimpanKabar} className="space-y-5"><input type="text" required value={judulKabar} onChange={(e) => setJudulKabar(e.target.value)} placeholder="Judul Berita" className="w-full px-4 py-3 rounded-xl border focus:ring-2 focus:ring-green-500 outline-none" /><label className="cursor-pointer flex flex-col items-center justify-center py-6 bg-green-50 border-2 border-dashed border-green-300 rounded-xl hover:bg-green-100 transition-all"><span className="text-3xl mb-2">📸</span><span className="font-bold">Klik pilih foto (Bisa banyak)</span><input id="inputFotoKabar" type="file" accept="image/*" multiple onChange={(e) => setFotoKabarList(e.target.files)} className="hidden" /></label>{fotoKabarList && <p className="text-sm font-bold text-blue-600">✅ {fotoKabarList.length} foto siap.</p>}<textarea required rows={6} value={isiKabar} onChange={(e) => setIsiKabar(e.target.value)} placeholder="Isi Berita" className="w-full px-4 py-3 rounded-xl border focus:ring-2 focus:ring-green-500 outline-none"></textarea>{statusKabar && <div className="p-3 rounded-lg text-sm font-bold text-center border bg-green-50 text-green-700">{statusKabar}</div>}<button type="submit" disabled={isLoadingKabar} className="bg-green-600 text-white font-bold px-8 py-3 rounded-xl">{isLoadingKabar ? "Memproses..." : (editKabarId ? "Simpan Perubahan" : "Publikasikan")}</button></form></div><div className="bg-white p-6 rounded-3xl shadow-sm overflow-x-auto"><h4 className="text-xl font-bold mb-4">Riwayat Berita</h4><table className="min-w-full text-sm text-left"><thead className="bg-gray-50 border-b"><tr><th className="py-4 px-4">Judul</th><th className="py-4 px-4 text-center">Aksi</th></tr></thead><tbody>{riwayatKabar.map((item) => (<tr key={item.id} className="border-b"><td className="py-4 px-4 font-bold">{item.judul}</td><td className="py-4 px-4 text-center"><button onClick={() => mulaiEditKabar(item)} className="bg-blue-100 text-blue-700 px-3 py-1 rounded-lg mr-2 font-bold">Edit</button><button onClick={() => hapusKabar(item.id)} className="bg-red-100 text-red-700 px-3 py-1 rounded-lg font-bold">Hapus</button></td></tr>))}</tbody></table></div></div>)}
