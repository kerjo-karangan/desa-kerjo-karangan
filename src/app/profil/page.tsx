...
export default function ProfilDesa() {
  ...
  useEffect(() => {
    const ambilData = async () => {
      try {
        // 1. Ambil Teks Profil Utama
        const docRef = doc(db, "profil_desa", "utama");
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setProfil({ sejarah: docSnap.data().sejarah || "", visi_misi: docSnap.data().visi_misi || "" });
        }

        // 2. Ambil Susunan Aparatur Desa
        ...
        if (dataAparatur.length === 0) {
           setAparaturDesa([{ nama: "Belum Ada Data", jabatan: "Kepala Desa", foto: "" }]);
        } else {
           setAparaturDesa(dataAparatur);
        }

        // 3. Ambil Data UMKM / Potensi Desa (BARU)
        const qUmkm = query(collection(db, "potensi_desa"), orderBy("tanggal_input", "desc"));
        const snapUmkm = await getDocs(qUmkm);
        const dataUmkm: any[] = [];
        snapUmkm.forEach(doc => dataUmkm.push({ id: doc.id, ...doc.data() }));
        setDaftarUmkm(dataUmkm);
    ...
    <main className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-green-800 text-white py-16 md:py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-4">Profil, Lembaga & Potensi Desa</h1>
          <p className="text-lg md:text-xl text-green-100 max-w-2xl mx-auto font-light">Sejarah, arah perjuangan, susunan aparatur, serta produk unggulan kebanggaan Desa Kerjo.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <section className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            {loading ? (<div className="animate-pulse space-y-3"><div className="h-3 bg-gray-200 rounded w-full"></div><div className="h-3 bg-gray-200 rounded w-5/6"></div></div>) : (
              <p className="text-gray-600 leading-relaxed whitespace-pre-wrap text-justify">{profil.sejarah || "Sejarah desa belum dipublikasikan."}</p>
            )}
          </section>
          ...
          <section className="bg-gradient-to-br from-green-800 to-green-900 p-8 md:p-10 rounded-3xl shadow-lg text-white transform hover:-translate-y-1 transition-transform">
            ...
            {loading ? (<div className="animate-pulse space-y-3"><div className="h-3 bg-green-700 rounded w-full"></div></div>) : (
              <p className="text-green-50 leading-relaxed whitespace-pre-wrap text-justify font-medium">{profil.visi_misi || "Visi dan Misi desa belum dipublikasikan."}</p>
            )}
          </section>
        </div>

        {/* STRUKTUR PEMERINTAH DESA (DENGAN PERBAIKAN PROXY GAMBAR) */}
        <section>
          <div className="text-center mb-12">
            ...
            <p className="text-gray-500 mt-3 max-w-2xl mx-auto">Susunan aparatur pelayan masyarakat Desa Kerjo yang ditarik secara otomatis dari database sistem pemerintahan.</p>
          </div>

          {!loading && aparaturDesa.length > 0 && (
            <>
              {/* Puncak Hierarki (Kades) */}
              <div className="flex justify-center mb-8">
                <div className="bg-white p-6 md:p-8 rounded-3xl shadow-md border-b-4 border-yellow-400 text-center w-full max-w-sm transform transition-transform hover:scale-105">
                  <div className="w-28 h-28 mx-auto bg-green-50 rounded-full flex items-center justify-center mb-4 border-4 border-green-500 shadow-inner overflow-hidden relative">
                    {aparaturDesa[0].foto ? (
                       // PERBAIKAN: Menambahkan Proxy CDN wsrv.nl
                       <img src={`https://wsrv.nl/?url=${aparaturDesa[0].foto}`} alt={aparaturDesa[0].nama} className="w-full h-full object-cover" />
                    ) : (
                       <span className="text-5xl absolute mt-2">👤</span>
                    )}
                  </div>
                ...
                {aparaturDesa.slice(1).map((aparatur, index) => (
                  <div key={index} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 text-center hover:border-green-400 hover:shadow-md transition-all group">
                    <div className="w-16 h-16 mx-auto bg-gray-50 rounded-full flex items-center justify-center mb-3 overflow-hidden shadow-sm border border-gray-200 group-hover:border-green-300">
                      {aparatur.foto ? (
                         // PERBAIKAN: Menambahkan Proxy CDN wsrv.nl
                         <img src={`https://wsrv.nl/?url=${aparatur.foto}`} alt={aparatur.nama} className="w-full h-full object-cover" />
                      ) : (
        ...
        {/* KATALOG UMKM / POTENSI DESA (FITUR BARU) */}
        <section className="pt-10 border-t border-gray-200">
          <div className="text-center mb-12">
            ...
            <p className="text-gray-500 mt-3 max-w-2xl mx-auto">Dukung perekonomian lokal dengan membeli produk-produk unggulan karya warga Desa Kerjo.</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white p-10 rounded-3xl text-center border border-dashed border-gray-300">
              ...
              <p className="text-gray-500 font-medium">Belum ada produk UMKM yang didaftarkan.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {daftarUmkm.map((umkm) => (
                <div key={umkm.id} className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col hover:shadow-lg transition-shadow">
                  {/* Foto Produk dengan Proxy */}
                  <div className="h-48 bg-gray-100 relative overflow-hidden">
                    {umkm.foto ? (
                      <img src={`https://wsrv.nl/?url=${umkm.foto}`} alt={umkm.nama_produk} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <a 
                      ...
                      className="w-full bg-green-50 hover:bg-green-600 text-green-700 hover:text-white border border-green-200 hover:border-green-600 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2"
                    >
                      <span className="text-lg">💬</span> Hubungi Penjual
                    </a>
                  </div>
                </div>
              ))}
