...
function LayananContent() {
  ...
  // FUNGSI: KIRIM PERMOHONAN SURAT
  const handleKirimSurat = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoadingSurat(true);
    setStatusSubmitSurat("Mengirim permohonan ke server desa...");
    ...
    try {
      ...
      setStatusSubmitSurat(`✅ Berhasil! Mohon simpan KODE RESI Anda: ${kodeResi}`);
      setNikSurat("");
      setNamaSurat("");
      setJenisSurat("");
      setKeperluanSurat("");
  ...
  // FUNGSI: KIRIM PENGADUAN (DGN IMGBB & PROXY CDN)
      ...
      <div className="bg-green-800 text-white py-16 relative overflow-hidden shadow-lg">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center max-w-3xl">
          ...
          <p className="text-base md:text-lg text-green-100 font-light leading-relaxed">
            Pusat pelayanan administrasi digital terpadu Desa Kerjo. Urus surat, lacak status, dan sampaikan aspirasi Anda dari mana saja.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-10 max-w-4xl flex-grow">
        ...
        <div className="flex flex-wrap md:flex-nowrap justify-center gap-2 md:gap-4 mb-8">
          <button 
            ...
            className={`flex-1 md:flex-none px-6 py-3 md:py-4 rounded-xl font-bold text-sm md:text-base transition-all duration-300 shadow-sm flex items-center justify-center gap-2 ${tabAktif === "surat" || !tabAktif ? "bg-green-600 text-white shadow-md transform -translate-y-1" : "bg-white text-gray-600 hover:bg-green-50 border border-gray-200"}`}
          >
          ...
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 animate-fade-in">
            ...
            <form onSubmit={handleKirimSurat} className="space-y-6">
              ...
              <div>
                ...
                <select required value={jenisSurat} onChange={(e)=>setJenisSurat(e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none bg-gray-50 focus:bg-white cursor-pointer font-medium text-gray-800">
                  <option value="">-- Pilih Jenis Surat --</option>
                  <option value="Surat Keterangan Usaha (SKU)">Surat Keterangan Usaha (SKU)</option>
                  <option value="Surat Pengantar SKCK">Surat Pengantar Kelakuan Baik (SKCK)</option>
              ...
              {statusSubmitSurat && (
                <div className={`p-4 rounded-xl font-bold text-center border ${statusSubmitSurat.includes("❌") ? "bg-red-50 text-red-700 border-red-200" : "bg-green-100 text-green-800 text-lg border-green-300"}`}>
                  {statusSubmitSurat}
                </div>
              )}
              ...
              <button type="submit" disabled={isLoadingSurat} className={`w-full text-white font-bold py-4 rounded-xl shadow-md transition-all text-lg ${isLoadingSurat ? "bg-gray-400" : "bg-green-600 hover:bg-green-700 hover:-translate-y-1"}`}>
                {isLoadingSurat ? "Memproses Permohonan..." : "Kirim Pengajuan Surat"}
              </button>
            </form>
          </div>
        )}
        ...
        {tabAktif === "status" && (
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 animate-fade-in">
             <div className="mb-8 text-center max-w-xl mx-auto">
              ...
              <p className="text-gray-500 text-sm">Masukkan NIK atau Kode Resi pengajuan Anda untuk melihat status terkini berkas Anda.</p>
            </div>
            
            <form onSubmit={handleLacakBerkas} className="flex flex-col md:flex-row gap-4 mb-10 max-w-2xl mx-auto">
              <input type="text" required value={lacakNik} onChange={(e)=>setLacakNik(e.target.value)} placeholder="Masukkan NIK / Kode Resi Anda..." className="flex-1 px-4 py-4 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none bg-gray-50 focus:bg-white font-bold text-center md:text-left text-lg text-gray-800 tracking-wider" />
              <button type="submit" disabled={isLoadingLacak} className="bg-gray-800 hover:bg-gray-900 text-white px-8 py-4 rounded-xl font-bold shadow-md transition-all text-lg">
                {isLoadingLacak ? "Mencari Data..." : "Lacak Berkas"}
              </button>
            </form>

            {statusLacak && <p className="text-center font-bold text-red-600 mb-6 bg-red-50 p-4 rounded-xl border border-red-200">{statusLacak}</p>}
            ...
            {hasilLacak ? (
              <div className="border border-green-200 rounded-2xl p-6 md:p-10 bg-green-50 shadow-inner">
                <div className="flex flex-col md:flex-row justify-between mb-8 pb-6 border-b border-green-200 gap-4">
                  <div>
                    ...
                    <p className="text-gray-700 font-medium mt-1">{hasilLacak.nama} <span className="text-gray-400">({hasilLacak.nik})</span></p>
                  </div>
                  <div className="text-left md:text-right">
                    <p className="text-sm text-green-700 font-bold uppercase tracking-wider mb-2">Kode Resi</p>
                    <span className="bg-white border-2 border-green-400 px-4 py-2 rounded-xl font-mono font-bold text-green-800 shadow-sm text-lg tracking-widest">{hasilLacak.resi}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between w-full max-w-2xl mx-auto relative mt-4">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center font-bold shadow-md text-lg">✓</div>
                    <span className="text-xs md:text-sm font-bold mt-3 text-green-700">Diajukan</span>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-md transition-colors text-lg ${hasilLacak.status_berkas === "Verifikasi" || hasilLacak.status_berkas === "Selesai" ? "bg-green-500 text-white" : "bg-gray-300 text-gray-500 border-2 border-white"}`}>
                      {hasilLacak.status_berkas === "Verifikasi" || hasilLacak.status_berkas === "Selesai" ? "✓" : "2"}
                    </div>
                    <span className={`text-xs md:text-sm font-bold mt-3 ${hasilLacak.status_berkas === "Verifikasi" || hasilLacak.status_berkas === "Selesai" ? "text-green-700" : "text-gray-500"}`}>Verifikasi</span>
                  </div>
                  
                  <div className="flex flex-col items-center">
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold shadow-md transition-colors text-lg ${hasilLacak.status_berkas === "Selesai" ? "bg-green-500 text-white" : "bg-gray-300 text-gray-500 border-2 border-white"}`}>
                      {hasilLacak.status_berkas === "Selesai" ? "✓" : "3"}
                    </div>
                    <span className={`text-xs md:text-sm font-bold mt-3 ${hasilLacak.status_berkas === "Selesai" ? "text-green-700" : "text-gray-500"}`}>Siap Diambil</span>
                  </div>
                </div>
              </div>
            ) : (
              !statusLacak && (
                <div className="border-2 border-dashed border-gray-300 rounded-2xl p-10 flex flex-col items-center justify-center text-center bg-gray-50">
                  ...
                  <p className="text-gray-500 font-medium">Silakan lakukan pencarian di atas untuk memunculkan linimasa proses berkas Anda.</p>
                </div>
              )
            )}
          </div>
        ...
        {tabAktif === "pengaduan" && (
          <div className="bg-white p-6 md:p-10 rounded-3xl shadow-sm border border-gray-100 animate-fade-in">
            <div className="mb-8 border-b pb-4">
              ...
              <p className="text-gray-500 text-sm">Laporkan masalah infrastruktur desa, keluhan pelayanan, atau usulan langsung ke Pemerintah Desa.</p>
            </div>
            <form onSubmit={handleKirimPengaduan} className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Judul Laporan / Keluhan</label>
                <input type="text" required value={judulPengaduan} onChange={(e)=>setJudulPengaduan(e.target.value)} placeholder="Contoh: Saluran Irigasi Dusun Krajan Tersumbat" className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none bg-gray-50 focus:bg-white text-gray-800 font-medium" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Deskripsi Lengkap Kejadian</label>
                <textarea required rows={5} value={isiPengaduan} onChange={(e)=>setIsiPengaduan(e.target.value)} placeholder="Ceritakan secara rinci apa yang terjadi, lokasi spesifiknya, dll..." className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-green-500 outline-none bg-gray-50 focus:bg-white text-gray-800 leading-relaxed"></textarea>
              </div>
              <div>
                ...
                <label className="w-full flex flex-col items-center justify-center px-4 py-10 bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl hover:bg-gray-100 cursor-pointer transition-colors">
                  <span className="text-4xl mb-3">{fotoPengaduan ? '✅' : '📸'}</span>
                  <span className="font-bold text-gray-700 text-sm md:text-base">{fotoPengaduan ? fotoPengaduan.name : 'Klik untuk memilih foto dari perangkat Anda'}</span>
              ...
              <div className="flex items-center gap-3 bg-blue-50 p-4 rounded-xl border border-blue-100">
                <input type="checkbox" id="anonim" checked={isAnonim} onChange={(e)=>setIsAnonim(e.target.checked)} className="w-6 h-6 text-green-600 rounded cursor-pointer" />
                <label htmlFor="anonim" className="text-sm font-bold text-gray-800 cursor-pointer select-none flex flex-col">
                  <span>Kirim sebagai Anonim (Rahasiakan Identitas Saya)</span>
                  <span className="text-xs text-gray-500 font-normal mt-0.5">Nama Anda tidak akan ditampilkan di sistem dashboard admin.</span>
                </label>
              </div>

              {statusSubmitPengaduan && (
                <div className={`p-4 rounded-xl font-bold text-center border ${statusSubmitPengaduan.includes("❌") ? "bg-red-50 text-red-700 border-red-200" : "bg-blue-50 text-blue-800 border-blue-200 text-lg"}`}>
                  {statusSubmitPengaduan}
                </div>
              )}

              <button type="submit" disabled={isLoadingPengaduan} className={`w-full text-white font-bold py-4 rounded-xl shadow-md transition-all text-lg ${isLoadingPengaduan ? "bg-gray-400 cursor-not-allowed" : "bg-gray-900 hover:bg-black hover:-translate-y-1"}`}>
                {isLoadingPengaduan ? "Menganalisis & Mengirim Laporan..." : "Kirim Laporan Pengaduan"}
              </button>
            </form>
          </div>
        )}
